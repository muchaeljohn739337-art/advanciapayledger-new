...
const FinancialProjections = () => {
  ...
  const generateMonthlyData = () => {
    ...
    for (let month = 0; month <= 12; month++) {
      ...
      const netNew = Math.round(
        (currentMRR * currentAssumptions.monthlyGrowth) /
          currentAssumptions.avgRevenuePerFacility,
      );
  ...
  const generateAnnualData = () => {
    ...
    const year1Ebitda = monthlyData
      .slice(1)
      .reduce((sum, m) => sum + m.ebitda, 0);
    ...
    const data: Array<{
    ...
    }> = [
      {
        ...
        ebitda: year1Ebitda,
        cashFlow: year1Ebitda,
        ebitdaMargin: year1Ebitda / year1Revenue,
      },
    ];
    ...
    const yearlyGrowthRates: Record<ScenarioType, number[]> = {
    ...
    for (let year = 2; year <= 5; year++) {
      ...
      const opExRate = Math.max(0.6 - year * 0.05, 0.4);
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-yellow-50 p-4 rounded-lg">
            ...
            <div className="text-2xl font-bold text-yellow-600">
              {formatPercent(metrics.year5EbitdaMargin)}
            </div>
          </div>
        </div>
              ...
              <tbody>
                {monthlyData.map((month, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? "bg-gray-50" : ""}>
                    ...
                    <td
                      className={`px-4 py-2 text-right ${month.ebitda < 0 ? "text-red-600" : "text-green-600"}`}
                    >
                      {formatCurrency(month.ebitda)}
                    </td>
                    <td
                      className={`px-4 py-2 text-right ${month.cashFlow < 0 ? "text-red-600" : "text-green-600"}`}
                    >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {annualData.map((year) => (
                  <tr
                    key={year.year}
                    className={year.year % 2 === 0 ? "bg-gray-50" : ""}
                  >
                    ...
                    <td
                      className={`px-4 py-2 text-right ${year.ebitda < 0 ? "text-red-600" : "text-green-600"}`}
                    >
                      {formatCurrency(year.ebitda)}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {year.ebitdaMargin
                        ? formatPercent(year.ebitdaMargin)
                        : "-"}
                    </td>
                    <td
                      className={`px-4 py-2 text-right ${year.cashFlow < 0 ? "text-red-600" : "text-green-600"}`}
                    >
